...
The multi-season `league_history.json` and `auction_prices.json` archives live
under `docs/data/` (not `data/`) — see [League History Archive](#league-history-archive).

## League History Archive

`docs/data/league_history.json` is a season-by-season archive of the league's history
...
handles). It lives under `docs/data/` — alongside `auction_prices.json` (the
2016–present auction salaries, grouped by owner) — so the standalone League
History page can serve both directly from GitHub Pages. This file is the source
of truth — it is committed and edited in place.
...
`docs/data/league_history.json` (replacing it if the year is already present). It
